...
## 2025-03-05 - Tensor Loop Overheads
**Learning:** Performing tensor operations (like `cosine_similarity`) inside Python `for` loops across multiple items (like finding matches for speaker IDs or voice profiles) introduces high C++ to Python transition overhead and scales poorly compared to natively batched tensor operations.
**Action:** Always batch computations. Use `torch.stack()` to combine sequences of items into a single tensor, perform operations on the whole batch natively, and extract the best match using `torch.max()`. Ensure that lists are strictly validated as non-empty before calling `torch.stack` to avoid `RuntimeError`.
