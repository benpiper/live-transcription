...
## 2024-05-24 - Pre-compile Regex in UI Hot Paths
**Learning:** Instantiating `new RegExp()` inside hot render paths (like rendering transcription streams) causes performance regressions due to repeated compilation.
**Action:** Pre-compile regular expressions globally and cache transformed strings whenever their source configuration (like `watchwords`) changes.
