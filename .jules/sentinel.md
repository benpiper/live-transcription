...
## 2024-10-27 - [XSS bypass via Type Confusion in Sanitization Functions]
**Vulnerability:** The sanitization functions `escapeHtml`, `escapeJs`, and `escapeRegExp` in `static/app.js` returned early if `typeof unsafe !== 'string'`. If an array payload like `["<script>alert(1)</script>"]` was passed, it bypassed the type check (since it's an object, not a string) and was returned unsanitized. When interpolated into a template literal, JavaScript implicitly called `.toString()` on the array, executing the XSS.
**Learning:** Checking for `typeof === 'string'` is insufficient for sanitization functions if the value is later implicitly cast to a string by the runtime (e.g., in template literals). Array or object payloads can easily bypass string type checks.
**Prevention:** In sanitization functions, explicitly handle null/undefined values and then cast the input to a string (e.g., `String(unsafe)`) before performing sanitization, rather than returning non-string types as-is.
