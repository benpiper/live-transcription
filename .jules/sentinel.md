...
## 2026-05-08 - [Regex Injection / ReDoS in Vanilla JS Frontend]
**Vulnerability:** The vanilla JavaScript frontend dynamically created `new RegExp` objects using unescaped user input (watchwords) in the `highlightWatchwords` function.
**Learning:** Any user-controlled string used as a pattern in a dynamic regular expression constructor can lead to Regex Injection and Regular Expression Denial of Service (ReDoS) if special characters are not appropriately escaped.
**Prevention:** Always sanitize user-controlled strings using a function like `escapeRegExp` to neutralize special regex characters before interpolating them into a `new RegExp()` constructor.
