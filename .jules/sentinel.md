...
## 2025-03-05 - Fix XSS bypass via implicit toString() on non-string inputs
**Vulnerability:** DOM-based XSS vulnerability in frontend where sanitization functions (`escapeHtml`, `escapeJs`) bypassed inputs that were not strings (e.g., Arrays), allowing malicious payloads to execute when implicitly converted to strings during `innerHTML` insertion.
**Learning:** The previous implementation `if (typeof unsafe !== 'string') return unsafe;` was flawed because it didn't sanitize Array objects, which get concatenated directly into HTML.
**Prevention:** Always explicitly check for null/undefined, and cast inputs to string using `String(unsafe)` before running `.replace()` for sanitization.
