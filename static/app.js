...
let watchwords = [];
let compiledWatchwords = { lower: [], regexes: [] };
let theme = 'dark';
...
function compileWatchwords() {
    compiledWatchwords.lower = watchwords.map(w => w.toLowerCase());
    compiledWatchwords.regexes = watchwords.map(word => {
...
function checkWatchwords(text) {
    if (compiledWatchwords.lower.length === 0) return false;
    const lowerText = text.toLowerCase();
    return compiledWatchwords.lower.some(word => lowerText.includes(word));
}

function highlightWatchwords(text) {
    let result = escapeHtml(text || '');
    if (compiledWatchwords.regexes.length === 0) return result;
    for (const regex of compiledWatchwords.regexes) {
