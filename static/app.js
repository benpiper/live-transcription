        ...
        if (!response.ok) {
            if (response.status === 400) {
                ...
                if (historyAudio && historyAudio.length > 0) {
                    ...
                    showSystemAlert('warning', "Audio not available", "This segment is outside the 2-hour buffer window.");
                }
            } else {
...
if (notificationBtn) {
    notificationBtn.addEventListener('click', async () => {
        if (!("Notification" in window)) {
            showSystemAlert('error', "Notifications Not Supported", "This browser does not support desktop notifications.");
            return;
        }
        ...
        if (Notification.permission === 'denied') {
            ...
            const msg = `Notifications are blocked by your ${browser} settings.\n\n` +
                ...
                `3. Refresh this page.`;
            showSystemAlert('error', "Notifications Blocked", msg.replace(/\n/g, '<br>'));
            return;
        }
