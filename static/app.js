...
let watchwords = [];
let watchwordsLowerCache = [];
let watchwordsRegexCache = [];
let theme = 'dark';
...
function updateWatchwordCaches() {
    watchwordsLowerCache = watchwords.map(w => w.toLowerCase());
    watchwordsRegexCache = watchwords.map(w => new RegExp(`(${escapeRegExp(w)})`, 'gi'));
}

function checkWatchwords(text) {
    if (watchwordsLowerCache.length === 0) return false;
    const lowerText = text.toLowerCase();
    return watchwordsLowerCache.some(lowerWord => lowerText.includes(lowerWord));
}

function highlightWatchwords(text) {
    let result = escapeHtml(text || '');
    if (watchwordsRegexCache.length === 0) return result;
    for (const regex of watchwordsRegexCache) {
