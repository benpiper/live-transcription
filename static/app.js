...
let watchwords = [];
let lowercasedWatchwords = [];
let compiledWatchwordsRegex = [];
let theme = 'dark';
...
function updateWatchwordsCache() {
    lowercasedWatchwords = watchwords.map(w => w.toLowerCase());
    compiledWatchwordsRegex = watchwords.map(w => new RegExp(`(${escapeRegExp(w)})`, 'gi'));
}

function checkWatchwords(text) {
    if (watchwords.length === 0) return false;
    const lowerText = text.toLowerCase();
    return lowercasedWatchwords.some(word => lowerText.includes(word));
}

function highlightWatchwords(text) {
    let result = escapeHtml(text || '');
    if (watchwords.length === 0) return result;
    for (const regex of compiledWatchwordsRegex) {
