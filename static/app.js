...
function updateCompiledWatchwords() {
    lowerWatchwords = watchwords.map(w => w.toLowerCase());
...
function checkWatchwords(text) {
    if (lowerWatchwords.length === 0) return false;
    const lowerText = text.toLowerCase();
    return lowerWatchwords.some(word => lowerText.includes(word));
}

function highlightWatchwords(text) {
    let result = escapeHtml(text || '');
    if (watchwordsRegExps.length === 0) return result;
