...
// Performance optimization: Pre-compute lowercase versions and regex objects
// so we don't repeatedly allocate strings or instantiate objects in rendering loops.
function updateWatchwordsCache() {
    watchwordsLower = watchwords.map(w => w.toLowerCase());
    watchwordRegexes = watchwords.map(word => new RegExp(`(${escapeRegExp(word)})`, 'gi'));
}
