...
let cachedWatchwordsRegex = null;

function updateWatchwordsCache() {
    if (watchwords.length === 0) {
        cachedWatchwordsRegex = null;
        return;
    }
    const sorted = [...watchwords].sort((a, b) => b.length - a.length);
    const escaped = sorted.map(escapeRegExp);
    cachedWatchwordsRegex = new RegExp(`(${escaped.join('|')})`, 'gi');
}

function checkWatchwords(text) {
    if (!cachedWatchwordsRegex || !text) return false;
    cachedWatchwordsRegex.lastIndex = 0;
    return cachedWatchwordsRegex.test(text);
}

function highlightWatchwords(text) {
    let result = escapeHtml(text || '');
    if (!cachedWatchwordsRegex) return result;
    cachedWatchwordsRegex.lastIndex = 0;
    return result.replace(cachedWatchwordsRegex, '<mark class="watchword-highlight">$1</mark>');
}
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 400) {
                showSystemAlert("error", "Audio not available", "This segment is outside the audio buffer window (2 hours).");
            } else {
        ...
        if (!response.ok) {
            if (response.status === 400) {
                ...
                if (historyAudio && historyAudio.length > 0) {
                    ...
                    showSystemAlert("error", "Audio not available for playback", "This segment is outside the 2-hour buffer window.");
                }
            } else {
...
if (notificationBtn) {
    notificationBtn.addEventListener('click', async () => {
        if (!("Notification" in window)) {
            showSystemAlert("error", "This browser does not support desktop notifications.");
            return;
        }
        ...
        if (Notification.permission === 'denied') {
            ...
            const msg = `Notifications are blocked by your ${browser} settings.\n\n` +
                ...
                `3. Refresh this page.`;
            showSystemAlert("error", "Notifications blocked", msg);
            return;
        }
...
(function initAudioProcessingControls() {
    ...
    resetBtn.addEventListener('click', () => {
        if (!confirm("Are you sure you want to reset audio processing settings to defaults?")) return;
