...
async function downloadSegment(id) {
    ...
    if (!item) {
        ...
        showSystemAlert('error', 'Download Failed', 'Transcript item not found');
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 400) {
                ...
                showSystemAlert('error', 'Download Failed', 'Failed to download audio segment');
            }
            return;
        }
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 400) {
                ...
                showSystemAlert('error', 'Playback Failed', 'Failed to load audio segment');
            }
            return;
        }
...
if (notificationBtn) {
    notificationBtn.addEventListener('click', async () => {
        if (!("Notification" in window)) {
            showSystemAlert('warning', 'Notifications Unsupported', 'This browser does not support desktop notifications.');
            return;
        }
