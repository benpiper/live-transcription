...
function highlightWatchwords(text) {
    ...
    if (watchwordsRegexes.length === 0) return result;
    for (const cache of watchwordsRegexes) {
        result = result.replace(cache.regex, '<mark class="watchword-highlight">$1</mark>');
    }
    return result;
}
