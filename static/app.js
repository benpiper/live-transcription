...
function updateWatchwordCaches() {
    ...
    watchwordRegexes = watchwords.map(w => new RegExp(`(${escapeRegExp(w)})`, 'gi'));
}
...
function highlightWatchwords(text) {
    ...
    for (let i = 0; i < watchwords.length; i++) {
        result = result.replace(watchwordRegexes[i], '<mark class="watchword-highlight">$1</mark>');
    }
    return result;
}
