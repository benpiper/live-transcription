...
let watchwords = [];
let watchwordsRegexCache = []; // Cache of compiled regular expressions
let watchwordsLowerCache = []; // Cache of lowercased watchwords
let theme = 'dark';
...
function updateWatchwordsCache() {
    watchwordsRegexCache = watchwords.map(word => {
        const escapedWord = escapeRegExp(word);
        return new RegExp(`(${escapedWord})`, 'gi');
    });
    watchwordsLowerCache = watchwords.map(word => word.toLowerCase());
}
