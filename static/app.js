...
let watchwords = [];
let watchwordLowerCache = [];
let watchwordRegexCache = [];
let theme = 'dark';
...
function updateWatchwordCaches() {
    watchwordLowerCache = watchwords.map(w => w.toLowerCase());
    watchwordRegexCache = watchwords.map(word => {
...
function checkWatchwords(text) {
    ...
    return watchwordLowerCache.some(word => lowerText.includes(word));
}

function highlightWatchwords(text) {
    let result = escapeHtml(text || '');
    if (watchwords.length === 0) return result;
    for (const regex of watchwordRegexCache) {
