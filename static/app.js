...
// Performance Optimization: Cache lowercased strings and compiled Regex objects
// This prevents redundant string manipulations and expensive RegExp compilations
// during the UI rendering hot paths (checkWatchwords and highlightWatchwords)
function updateWatchwordsCache() {
    watchwordsLower = watchwords.map(w => w.toLowerCase());
    watchwordsRegex = watchwords.map(w => new RegExp(`(${escapeRegExp(w)})`, 'gi'));
}
...
function highlightWatchwords(text) {
    ...
    if (watchwordsRegex.length === 0) return result;
    for (const regex of watchwordsRegex) {
